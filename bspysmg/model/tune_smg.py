...
def main(num_samples=10, max_num_epochs=15):
    ...
    hidden_sizes = []
    hidden_sizes.append(tune.choice([90, 45, 25, 10]))
    hidden_sizes.append(tune.choice([45, 25]))
    hidden_sizes.append(tune.choice([45, 25]))
    hidden_sizes.append(tune.choice([90, 45, 25, 10]))
    configs["processor"]["model_architecture"]["hidden_sizes"] = hidden_sizes
    ...
    result = tune.run(
        tune.with_parameters(generate_surrogate_model),
